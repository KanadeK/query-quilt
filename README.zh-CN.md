...
当前版本：**v0.1.0**。本地开发需要 Node.js 22 或更新版本。
...
v0.1.0 不承诺公开运行时 API。TypeScript 模块为测试和后续拆包而组织，但外部消费者应把它们
视为内部实现。
...
v0.1.0 基线包含 **12 个文件中的 58 项单元/集成测试**和 **8 项 Chromium E2E 测试**。
...
应用会拒绝超过 512 MiB 的文件；但 v0.1.0 使用内存执行，受限设备的实际可用上限可能低很多。
